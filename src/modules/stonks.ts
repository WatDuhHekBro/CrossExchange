import {Random, isType, select, GenericJSON, GenericStructure, pluralise, pluraliseSigned, split, random} from "../lib";
import {Stonks, Storage} from "../structures";
...
function getEventEmbed(
    headline: string,
    description: string,
    changes: {[market: string]: number},
    ...
    for (const tag in changes) {
        ...
        effects.push({
            ...
            value:
                delta !== 1
                    ? `Now headed towards being around ${decimal}% as valuable as before.`
        ...
        if (data && isType(data.effects, Object)) {
            for (const tag in data.effects) {
                ...
                if (isType(market, Array)) {
                    if (isType(market[0], Number) && isType(market[1], Number)) this.effects = data.effects;
                    else console.warn(`Tag "${tag}" of the selected effect is invalid!`, data.effects);
                }
            }
        }
    }
...
export class StonksStructure extends GenericStructure {
    ...
    constructor(data: GenericJSON) {
        ...
        for (const tag in StandardMarkets)
            this.markets[tag] = new Market(
                list[tag] ? Object.assign(list[tag], StandardMarkets[tag]) : StandardMarkets[tag]
            );

        if (isType(data.guilds, Object))
            for (const id in data.guilds) this.guilds[id] = new GuildUpdater(data.guilds[id]);
    }

    public getMarket(tag: string): Market | null {
        if (tag in this.markets) return this.markets[tag];
        else return null;
    }
    ...
    public async triggerStonks(client: Client): Promise<any> {
        ...
        for (const guildID in this.guilds) {
            ...
            if (!guild) return console.warn(`Guild "${guildID}" not found! Ignoring this guild.`);
            ...
            if (!channel)
                return console.warn(
                    `Channel "${container.channel}" of guild "${guild.id}" is not a valid channel ID! Ignoring this guild.`
                );
            if (channel.type !== "text")
                return console.warn(
                    `Channel "${channel.id}" of guild "${guild.id}" is not a text channel! Ignoring this guild.`
                );
            if (stored.length !== total)
                console.warn(
                    `The length of the generated embed (${total}) isn't the same as the total amount of messages provided (${stored.length}). Using the amount of messages provided, meaning some data points might be cut off.`
                );
            ...
            if (stored.length === 1) {
                ...
                textChannel.messages
                    ...
                    .catch(() => {
                        console.error(
                            `"${messageID}" isn't a valid message ID in channel "${container.channel}", guild "${guild.id}"!`
                        );
                    });
    ...
    public async triggerEvent(client: Client): Promise<any> {
        ...
        for (const guildID in this.guilds) {
            if (!client.guilds.cache.has(guildID)) return delete this.guilds[guildID];
    ...
    public async addGuild(channel: TextChannel) {
        ...
        if (total === 1) messages.push((await channel.send("Market Values", embeds[0])).id);
        else if (total > 1) {
        ...
        const event = (
            await channel.send(
                "Latest Event",
                getEventEmbed(
                    "There is currently no active headline.",
                    "Events will appear here as time goes on.",
                    {},
...
function loadEvents(data: GenericJSON[]): Event[] {
    const list: Event[] = [];
    for (const entry of data) list.push(new Event(entry));
    return list;
}

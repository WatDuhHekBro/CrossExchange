...
module.exports = {
	run($)
	{
		...
		let embed = {
			title: "Market Values",
			description: "The current iteration of markets' values and data (for testing purposes only).",
			color: "#00ad2e",
			fields: []
		};
		...
		for(let market in markets)
		{
			...
			let oldValue = Math.round(selected.value);
			...
			let displayValue = Math.round(selected.value);
			embed.fields.push({
				name: selected.name || "",
				value: displayValue + (displayValue === 1 ? " credit, " : " credits, ") + (displayValue - oldValue) + " credit change"
			});

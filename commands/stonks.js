module.exports = {
	...
	common:
	{
		buy: function($, s, amount)
		{
			let stonks = $.lib.loadJSON('stonks');
			let user = $.lib.get($.lib.loadJSON('users'), $.author.id, {});
			let market = stonks.markets[s];
			let value = Math.round(market.value);
			
			if($.lib.get(user, 'money', 0) >= (value * amount))
			{
				$.lib.get(user, 'stonks', {});
				let userAmount = $.lib.get(user.stonks, s, 0);
				user.stonks[s] += userAmount + amount;
				user.money -= value * amount;
				market.invested += amount;
				let stockName = $.lib.pluraliseWithNumber(amount, 'stock', 's');
				let stockNameInvested = $.lib.pluraliseWithNumber(market.invested, 'stock', 's');
		...
		sell: function($, s, amount)
		{
			...
			if($.lib.get(user.stonks, s, 0) > 0)
			{
				user.stonks[s] -= amount;
				...
				let stockName = $.lib.pluraliseWithNumber(amount, 'stock', 's');
				let stockNameInvested = $.lib.pluraliseWithNumber(market.invested, 'stock', 's');
				let creditName = $.lib.pluraliseWithNumber(value * amount, 'credit', 's');

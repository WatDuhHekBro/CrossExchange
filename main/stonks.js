...
module.exports = {
	...
	calculate(markets)
	{
		...
		for(let market in markets)
		{
			...
			selected.cycle += Math.random() * Math.random() * Math.random() * selected.amplitude * (Math.random() < selected.volatility ? 2 : 1);
			selected.cycle = selected.cycle > 1 ? (selected.cycle - 2) : selected.cycle;
			...
			selected.trend += trendMultiplier * (selected.amplitude * (Math.sin(selected.cycle * Math.PI) + 1)) - lib.rand(trendBuffer, trendMultiplier - 1 - trendBuffer); // However, even log base 10 generates multipliers too high. A small trend could easily spiral out into infinity. So I want to control that.
			console.log(selected.cycle, selected.trend);
